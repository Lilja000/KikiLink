...
describe("room Blossom character positioning", () => {
  ...
  it("draws through the native icon path and never leaves a normal-play DOM overlay", () => {
    ...
    expect(draw).toHaveBeenCalledWith(BLOSSOM_ICON_DATA_URL, 310, 42.5, 17.5, 17.5);
  ...
  it("retains a cached vector fallback when BC's page-owned image helpers are unavailable", () => {
    ...
    Reflect.deleteProperty(globalThis, "DrawImageResize");
    Reflect.deleteProperty(window, "DrawImageResize");
...
describe("room Blossom settings-armed dragging", () => {
  ...
  it("ignores normal pointer input and persists one character-relative drag when armed", () => {
    ...
    expect(settings.get().ui.roomBadge.position).toEqual({ x: 0.96, y: 0.135 });
