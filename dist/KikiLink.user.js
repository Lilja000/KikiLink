...
`;
  ...
  var LinkChatView = class {
    constructor(adapter, service, settings, version) {
    ...
    #conversationButton(conversation) {
      ...
      const preview = conversation.lastMessage ? `${prefix}${conversation.lastMessage}` : `Member ${conversation.peerNumber}`;
      const main = element(
        "div",
...
${expanded}` : expanded;
    ...
    #updateCounter() {
      ...
      this.#counter.textContent = `${count}/1000 \xB7 Ctrl+Enter`;
      this.#counter.dataset.over = String(count > 1e3);
    }
    #applyTheme(settings) {
